Feature: Process assignments
  ...
  Background: 
    ...
    Given the following assignments exist in the system:
      | memberEmail        | guideEmail   | startDay  | endDay  |
      | alice@gmail.com    | bob@ad.atl   |         1 |       3 |
      | charlie@hotmail.ca |              |         1 |       3 |
      | john@hotmail.ca    | diana@ad.atl |         1 |       3 |
      | emily@hotmail.ca   | bob@ad.atl   |         4 |       5 |
  ...
  Scenario Outline: Attempt to pay for trip with an email address that does not exist
    ...
    Examples: 
      | email            | code         | error                                                     |
      | steve@yahoo.com  | DIVESAFE2022 | Member with email address steve@yahoo.com does not exist  |
