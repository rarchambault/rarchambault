 ...
 *
 * @author Harrison Wang, Jimmy Sheng, Michael Grieco, Yida Pan, Annie Kang,
 *         Sibo Huang
 *
 */
public class ViewAssignmentStepDefinitions {
	...
	public void the_following_dive_safe_system_exists_p5(io.cucumber.datatable.DataTable dataTable) {
		/*
		 * Get the data for the DiveSafe instance from the Cucumber Feature file. Note:
		 * it is assumed that there will only be one element in the list returned from
		 * dataTable.asMaps(). This is because there can be only one instance of
		 * DiveSafe in the system at any given time. For this reason, we simply grab the
	...
	public void the_following_hotels_exist_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
	...
	public void the_following_guides_exist_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
	...
	public void the_following_equipment_exists_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
	...
	public void the_following_equipment_bundles_exist_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
	...
	public void the_following_members_exist_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
		List<Map<String, String>> rows = dataTable.asMaps();
		// add each member
		for (var row : rows) {
			String email = row.get("email");
			...
			var member = diveSafe.addMember(email, password, name, emergencyContact, numDays, guideRequired,
					hotelRequired);
			/*
			 * Parse items and quantities. The null check is necessary in case a member
			 * doesn't wish to rent any equipment (e.g. Mohamed Farah in
			 * ViewAssignment.feature, line 53)
			 */
	...
	public void the_following_assignments_exist_in_the_system_p5(io.cucumber.datatable.DataTable dataTable) {
		List<Map<String, String>> cucumberData = dataTable.asMaps();
	...
	public void the_following_assignment_information_shall_be_displayed_p5(io.cucumber.datatable.DataTable dataTable) {
		// convert data table to a map
		...
		for (int i = 0; i < outputDataList.size(); i++) {
			...
			assertEquals(Integer.parseInt(assignmentData.get("totalCostForGuide")), assignment.getTotalCostForGuide());
		}
	}

	/**
	 * Asserts that the strings are either equal, or one is empty and the other is
	 * null, or both are null.
	 */
