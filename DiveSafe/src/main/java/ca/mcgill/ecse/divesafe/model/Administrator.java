...
public class Administrator extends User {
	...
	public Administrator(String aEmail, String aPassword, DiveSafe aDiveSafe) {
		...
		if (!didAddDiveSafe) {
			throw new RuntimeException(
					"Unable to create administrator due to diveSafe. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
		}
	}
	...
	public boolean setDiveSafe(DiveSafe aNewDiveSafe) {
		...
		if (aNewDiveSafe == null) {
			// Unable to setDiveSafe to null, as administrator must always be associated to
			// a diveSafe
			return wasSet;
		}
		...
		if (existingAdministrator != null && !equals(existingAdministrator)) {
			// Unable to setDiveSafe, the current diveSafe already has a administrator,
	...
	public void delete() {
		...
		super.delete();
		DiveSafePersistence.save(existingDiveSafe);
	}
