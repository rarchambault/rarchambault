...
public class DiveSafe {
	...
	public boolean addGuideAt(Guide aGuide, int index) {
		...
		if (addGuide(aGuide)) {
			...
			if (index > numberOfGuides()) {
				index = numberOfGuides() - 1;
			}
			guides.remove(aGuide);
	...
	/* Code from template association_AddManyToOne */
	public Member addMember(String aEmail, String aPassword, String aName, String aEmergencyContact, int aNumDays,
			boolean aGuideRequired, boolean aHotelRequired) {
	...
	public boolean addEquipmentAt(Equipment aEquipment, int index) {
		...
		if (addEquipment(aEquipment)) {
			...
			if (index > numberOfEquipments()) {
				index = numberOfEquipments() - 1;
			}
			equipments.remove(aEquipment);
	...
	public boolean addBundleAt(EquipmentBundle aBundle, int index) {
		...
		if (addBundle(aBundle)) {
			...
			if (index > numberOfBundles()) {
				index = numberOfBundles() - 1;
			}
			bundles.remove(aBundle);
	...
	public boolean addBundleItemAt(BundleItem aBundleItem, int index) {
		...
		if (addBundleItem(aBundleItem)) {
			...
			if (index > numberOfBundleItems()) {
				index = numberOfBundleItems() - 1;
			}
			bundleItems.remove(aBundleItem);
	...
	public String toString() {
		return super.toString() + "[" + "numDays" + ":" + getNumDays() + "," + "priceOfGuidePerDay" + ":"
				+ getPriceOfGuidePerDay() + "]" + System.getProperties().getProperty("line.separator") + "  "
				+ "startDate" + "="
				+ (getStartDate() != null
						? !getStartDate().equals(this) ? getStartDate().toString().replaceAll("  ", "    ") : "this"
						: "null")
				+ System.getProperties().getProperty("line.separator") + "  " + "administrator = "
				+ (getAdministrator() != null ? Integer.toHexString(System.identityHashCode(getAdministrator()))
						: "null");
	}
}